...
for i = 1 : size( timeStampUtterance ) 
    ...
    for j = 1 : segmentNum
        ...
        audiowrite( [ './tempfile/wav/' num2str( i ), '_', num2str( j ) '.wav' ], ... temp wav path
    ...
    % the last (not complete) segment 
    timeStampSegment = [ timeStampSegment; startSegment, timeStampUtterance( i , 2 ), i , 0];
    
    audiowrite( [ './tempfile/wav/' num2str( i ), '_', num2str( j+1 ) '.wav' ], ... temp wav path
